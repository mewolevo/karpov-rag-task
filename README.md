...
На этом шаге мы научимся обрабатывать запросы пользователей, используя LLM API (например, OpenAI). С помощью созданного индекса курсов мы будем искать подходящие курсы, а затем генерировать ответ с учётом запроса пользователя.

### Что нужно сделать:
1. Преобразовать запрос пользователя в эмбеддинг (векторное представление текста).
2. Использовать FAISS индекс для поиска наиболее релевантных курсов.
3. Сгенерировать ответ с помощью LLM API, например, OpenAI.
4. Если API недоступно, возвращать результаты поиска по индексу курсов.
...
#### Инструменты:
- **FAISS**: Для быстрого поиска по эмбеддингу.
- **OpenAI API**: Для генерации более сложных ответов и рекомендаций.

### Шаги реализации

1. **Получение эмбеддинга запроса**: 
   Мы будем использовать модель для преобразования текста запроса в векторное представление (эмбеддинг).

2. **Поиск по индексу**: 
   На основе эмбеддинга запроса будем искать наиболее подходящие курсы в FAISS индексе.

3. **Обработка ответа с помощью LLM API**: 
...
### Задача
- Получить эмбеддинг для текста запроса с помощью функции get_query_embedding.
- Реализовать поиск курсов по эмбеддингу через FAISS индекс с помощью функции search_courses.
